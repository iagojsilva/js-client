...
import { ExistingTokenInfo, UpdatableToken } from '~/models';
import {
	APIContext,
...
export const makeUpdateOneToken = (context: APIContext) => {
	...
	return async (data: UpdatableToken): Promise<ExistingTokenInfo> => {
		...
		try {
			...
			const tokenInfoWithoutSecret = await parseJSONResponse<ExistingTokenInfo>(raw);
